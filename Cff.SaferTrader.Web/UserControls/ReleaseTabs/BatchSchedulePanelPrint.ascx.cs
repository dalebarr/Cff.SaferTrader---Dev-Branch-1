...
{
    public partial class BatchSchedulePanelPrint : System.Web.UI.UserControl, IScheduleTabView
    {
        public void Clear()
        {
        ...
        public void DisplayScheduleSummary(BatchScheduleFinanceInfo scheduleFinanceInfo)
        {
            ...
            if (scheduleFinanceInfo.FacilityType == 1)
            {

                if (scheduleFinanceInfo.CalculateTotalCharges() > 0)
                {
                    FactoredLiteral.Width = !FactoredLiteral01.Visible
                        ? Unit.Pixel(scheduleFinanceInfo.NonFactored > 0 ? 152 : 157)
                        : Unit.Pixel(104);
                } 
                else
                    if (scheduleFinanceInfo.Repurchase > 0)
                    {
                        FactoredLiteral.Width = !FactoredLiteral01.Visible
                            ? Unit.Pixel(scheduleFinanceInfo.NonFactored > 0 ? 157 : 153)
                            : Unit.Pixel(104);
                    }
                    else
                    {
                        FactoredLiteral.Width = !FactoredLiteral01.Visible
                            ? Unit.Pixel(scheduleFinanceInfo.NonFactored > 0 ? 157 : 176)
                            : Unit.Pixel(104);
                    }
            }
            else
            {
                FactoredLiteral.Width = !FactoredLiteral01.Visible ? Unit.Pixel(scheduleFinanceInfo.NonFactored > 0 ? 157 : 179) : Unit.Pixel(104);    
            }
